...
class SimpleThreadPool {
     ...
     * @param listCsvLines: List of strings, where in each string represents a line read from the CSV file.
     */
   public static void doTheAggregates(List<String> listCsvLines)
   {
       ...
       Map<String, List<Csv>> mapDomainToListCsvs = new HashMap<String, List<Csv>>();
       List<DomainRecord> listOfDomainRecords = new ArrayList<DomainRecord>();

       // Mapping the csv lines into a map, where the key is the domain name and the value is a list of Csv objects. Each of the csv objects represents a csv line.
       for (String resultLine : listCsvLines) {
           ...
           Csv csvRecord=new Csv(dRequestDateObject,sDomain);
           List<Csv> listOfCsvsForADomain= new ArrayList<Csv>();
           if(mapDomainToListCsvs.containsKey(sDomain))
               listOfCsvsForADomain=mapDomainToListCsvs.get(sDomain);
           listOfCsvsForADomain.add(csvRecord);
       ...
       for(Map.Entry<String, List<Csv>> resultLinesPerDomainEntry : mapDomainToListCsvs.entrySet()){
           DomainRecord domainRecord=  new DomainRecord(resultLinesPerDomainEntry.getKey());
           for(Csv csv:resultLinesPerDomainEntry.getValue()) {
               domainRecord.addRequestTime(csv.getRequestTime());
           }
